...
CREATE INDEX idx_audit_log_actions ON audit_logs(action_name, created_at);`
  ...
  return (
    <div className="bg-slate-900 text-slate-100 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden font-sans">
      {/* Tab Navigation header */}
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between border-b border-slate-800 bg-slate-950 px-6 py-4 gap-4">
        <div className="flex items-center gap-3">
          <Layers className="h-6 w-6 text-emerald-500" />
          <div>
            <h3 className="font-semibold text-lg text-white">Nibzo Architecture Blueprint</h3>
            <p className="text-xs text-slate-400">Production-grade specification ready for horizontal cluster expansion</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1 bg-slate-900 p-1 rounded-lg border border-slate-800">
          <button 
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${activeTab === 'architecture' ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            <CloudLightning className="inline h-3.5 w-3.5 mr-1" /> System Arch
          </button>
          <button 
            id="tab-btn-db"
            onClick={() => setActiveTab('database')} 
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${activeTab === 'database' ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            <Database className="inline h-3.5 w-3.5 mr-1" /> Database ({databaseSchemas.length} Tables)
          </button>
          <button 
            id="tab-btn-api"
            onClick={() => setActiveTab('apis')} 
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${activeTab === 'apis' ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            <Terminal className="inline h-3.5 w-3.5 mr-1" /> API Specs
          </button>
          <button 
            id="tab-btn-sec"
            onClick={() => setActiveTab('security')} 
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition ${activeTab === 'security' ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-white'}`}
          >
            <ShieldCheck className="inline h-3.5 w-3.5 mr-1" /> Security Rules
          </button>
        </div>
      </div>
      ...
      <div className="p-6">
        {activeTab === 'architecture' && (
          <div className="space-y-6">
            <div className="bg-slate-950 p-5 rounded-xl border border-slate-800 relative">
              <div className="absolute top-4 right-4 bg-emerald-500/10 text-emerald-400 text-2xs px-2.5 py-1 rounded-full font-mono border border-emerald-500/20">
                100% Commission-Free
              </div>
              <h4 className="text-white font-semibold text-lg">{sysArchitectureData.title}</h4>
              <p className="text-slate-400 text-sm mt-1">{sysArchitectureData.tagline}</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                {sysArchitectureData.components.map((c, i) => (
                  <div key={i} className="bg-slate-900 p-4 rounded-lg border border-slate-800/80 hover:border-slate-700 transition">
                    <h5 className="text-white font-medium text-sm flex items-center gap-2">
                      <span className="flex items-center justify-center h-5 w-5 rounded-full bg-emerald-500/10 text-emerald-400 text-2xs font-mono">{i+1}</span>
                      {c.title}
                    </h5>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800">
                <div className="text-emerald-400 font-mono text-xl font-bold">~ 40ms SLA</div>
                <div className="text-white font-medium text-xs mt-1">API Latency Limit</div>
                <p className="text-slate-400 text-2xs mt-1 leading-relaxed">Redis query bypass on active menus ensures rapid screen painting in heavy density environments.</p>
              </div>
              <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800">
                <div className="text-emerald-400 font-mono text-xl font-bold">100% Immutable</div>
                <div className="text-white font-medium text-xs mt-1">Financial Reconciliation</div>
                <p className="text-slate-400 text-2xs mt-1 leading-relaxed">System separates order volume from subscription logs; 100% of delivery fee flows to rider via UPI payout gateway.</p>
              </div>
              <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800">
                <div className="text-emerald-400 font-mono text-xl font-bold">Scale-to-Zero</div>
                <div className="text-white font-medium text-xs mt-1">Operational Cost Efficiency</div>
                <p className="text-slate-400 text-2xs mt-1 leading-relaxed">Docker microservice scaling logic drops execution environments automatically during quiet regional hours.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'database' && (
          <div className="space-y-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 mb-2">
              <div>
                <p className="text-xs text-slate-400">PostgreSQL Relational Schema. Formatted with foreign relations, optimized constraints, and multi-column indexes.</p>
              </div>
              <div className="relative w-full md:w-64">
                ...
                <input 
                  ...
                  className="w-full text-xs pl-9 pr-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 outline-none focus:border-emerald-500 transition"
                />
              </div>
            </div>

            <div className="max-h-[460px] overflow-y-auto space-y-4 pr-1 scrollbar-thin scrollbar-thumb-slate-800">
                ...
                <div key={idx} className="bg-slate-950 rounded-xl border border-slate-800 overflow-hidden">
                  <div className="bg-slate-900 px-4 py-2.5 flex justify-between items-center border-b border-slate-800">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-xs font-bold text-slate-200">TABLE: {schema.table}</span>
                    </div>
                    <button 
                      id={`copy-ddl-${schema.table}`}
                      onClick={() => triggerCopy(schema.ddl, schema.table)}
                      className="text-slate-500 hover:text-white transition flex items-center gap-1.5"
                    >
                      {copiedText === schema.table ? (
                        <>
                          <Check className="h-3 w-3 text-emerald-500" />
                          <span className="text-3xs text-emerald-500">Copied!</span>
                        </>
                      ) : (
                        <>
                          <Copy className="h-3 w-3" />
                          <span className="text-3xs">Copy SQL</span>
                        </>
                      )}
                    </button>
                  </div>
                  <div className="p-4">
                    <p className="text-slate-400 text-xs mb-3 italic">{schema.description}</p>
                    <pre className="font-mono text-3xs text-slate-300 leading-relaxed bg-slate-1000 p-3 rounded border border-slate-900/60 overflow-x-auto whitespace-pre">
                      {schema.ddl}
                    </pre>
                  </div>
                </div>
              ...
              {filteredSchemas.length === 0 && (
                <div className="text-center py-10 text-slate-500 text-xs text-center">
        ...
        {activeTab === 'apis' && (
          <div className="space-y-6">
            ...
            <div className="max-h-[460px] overflow-y-auto space-y-6 pr-1">
              {apis.map((group, gIdx) => (
                <div key={gIdx} className="space-y-4">
                  <h4 className="text-white font-semibold text-xs tracking-wider uppercase border-l-2 border-emerald-500 pl-2">
                    {group.section}
                  </h4>
                  
                  {group.endpoints.map((api, kIdx) => (
                    <div key={kIdx} className="bg-slate-950 rounded-xl border border-slate-800 overflow-hidden">
                      {/* EndPoint Head */}
                      <div className="bg-slate-900 px-4 py-3 flex flex-wrap gap-2 items-center justify-between border-b border-slate-800">
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-0.5 rounded text-3xs font-mono font-bold ${
                            api.method === 'POST' ? 'bg-blue-500/10 text-blue-400' : 'bg-emerald-500/10 text-emerald-400'
                          }`}>
                            {api.method}
                          </span>
                          <span className="font-mono text-xs text-slate-200">{api.url}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-3xs text-slate-500 font-mono">Auth: {api.auth}</span>
                        </div>
                      </div>
                      ...
                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <p className="text-slate-400 text-xs mb-3 leading-relaxed">{api.desc}</p>
                          <div className="space-y-1.5">
                            <span className="text-3xs font-mono uppercase text-slate-500 block">Payload Schema:</span>
                            <pre className="font-mono text-3xs text-slate-300 leading-tight bg-slate-925 p-3 rounded-lg border border-slate-800 overflow-x-auto whitespace-pre">
                              {api.payload}
                            </pre>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="space-y-1.5">
                            <span className="text-3xs font-mono uppercase text-slate-500 block">Success Mock Response (200 OK):</span>
                            <pre className="font-mono text-3xs text-emerald-300 leading-tight bg-emerald-950/20 p-3 rounded-lg border border-emerald-500/25 overflow-x-auto whitespace-pre">
                              {api.response}
                            </pre>
                          </div>
                        </div>
        ...
        {activeTab === 'security' && (
          <div className="space-y-6">
            <div className="bg-slate-950 p-5 rounded-xl border border-slate-800">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                <div className="p-3.5 bg-slate-900 rounded-lg border border-slate-800">
                  ...
                  <p className="text-slate-400 text-3xs mt-1.5 leading-relaxed">
            ...
            <div className="space-y-2">
              <span className="text-3xs font-mono uppercase text-slate-500 block">Typical MiddleWare Token Evaluation Code (Node/Express):</span>
              <pre className="font-mono text-3xs text-slate-300 leading-relaxed bg-slate-1000 p-4 rounded-xl border border-slate-950 overflow-x-auto whitespace-pre">
