...
CREATE INDEX idx_audit_log_actions ON audit_logs(action_name, created_at);`
  ...
  return (
    <div className="bg-white text-slate-900 rounded-2xl border border-slate-200 shadow-sm overflow-hidden font-sans">
      {/* Tab Navigation header */}
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between border-b border-slate-200 bg-[#FCFCFD] px-6 py-5 gap-4">
        <div className="flex items-center gap-3">
          <Layers className="h-6 w-6 text-orange-600" />
          <div>
            <h3 className="font-semibold text-lg text-slate-900">NIBZO Architecture Blueprint</h3>
            <p className="text-xs text-slate-500 mt-0.5">Production-grade specification ready for horizontal cluster expansion</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${activeTab === 'architecture' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-[#FCFCFD]'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${activeTab === 'database' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-[#FCFCFD]'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${activeTab === 'apis' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-[#FCFCFD]'}`}
          >
          ...
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${activeTab === 'security' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-[#FCFCFD]'}`}
          >
      ...
      <div className="p-8">
        {activeTab === 'architecture' && (
          <div className="space-y-6">
            <div className="bg-[#FCFCFD] p-6 rounded-2xl border border-slate-200 relative shadow-inner">
              ...
              <h4 className="text-slate-900 font-bold text-xl">{sysArchitectureData.title}</h4>
              <p className="text-slate-600 text-sm mt-2 max-w-2xl">{sysArchitectureData.tagline}</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-8">
                {sysArchitectureData.components.map((c, i) => (
                  <div key={i} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:border-orange-300 transition-colors">
                    <h5 className="text-slate-900 font-bold text-sm flex items-center gap-3">
                    ...
                    <p className="text-slate-600 text-xs mt-3 leading-relaxed">{c.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <div className="text-slate-900 font-bold text-xs mt-1.5 relative z-10">API Latency Limit</div>
                <p className="text-slate-500 text-[11px] mt-2 leading-relaxed relative z-10">Redis query bypass on active menus ensures rapid screen painting in heavy density environments.</p>
              </div>
              <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <div className="text-slate-900 font-bold text-xs mt-1.5 relative z-10">Financial Reconciliation</div>
                <p className="text-slate-500 text-[11px] mt-2 leading-relaxed relative z-10">System separates order volume from subscription logs; 100% of delivery fee flows to rider via UPI payout gateway.</p>
              </div>
              <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                ...
                <div className="text-slate-900 font-bold text-xs mt-1.5 relative z-10">Operational Cost Efficiency</div>
                <p className="text-slate-500 text-[11px] mt-2 leading-relaxed relative z-10">Docker microservice scaling logic drops execution environments automatically during quiet regional hours.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'database' && (
          <div className="space-y-5">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
              <div>
                <p className="text-sm text-slate-500 font-medium leading-relaxed">PostgreSQL Relational Schema. Formatted with foreign relations, optimized constraints, and multi-column indexes.</p>
              </div>
              <div className="relative w-full md:w-72">
                ...
                <input 
                  ...
                  className="w-full text-sm pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-900 outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition shadow-sm"
                />
              </div>
            </div>
            ...
            <div className="max-h-[500px] overflow-y-auto space-y-5 pr-2">
              {filteredSchemas.map((schema, idx) => (
                <div key={idx} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:border-slate-300 transition-colors">
                  <div className="bg-[#FCFCFD] px-5 py-3 flex justify-between items-center border-b border-slate-200">
                    <div className="flex items-center gap-2">
                      <Database className="h-4 w-4 text-orange-600" />
                      <span className="font-mono text-sm font-bold text-slate-900 tracking-tight">TABLE: {schema.table}</span>
                    </div>
                    <button 
                      id={`copy-ddl-${schema.table}`}
                      onClick={() => triggerCopy(schema.ddl, schema.table)}
                      className="text-slate-500 hover:text-slate-900 transition flex items-center gap-1.5 bg-white border border-slate-200 px-2 py-1 rounded-md shadow-sm"
                    >
                  ...
                  <div className="p-5">
                    <p className="text-slate-600 text-sm mb-4 leading-relaxed">{schema.description}</p>
                    <pre className="font-mono text-xs text-slate-800 leading-relaxed bg-[#FCFCFD] p-4 rounded-lg border border-slate-100 overflow-x-auto whitespace-pre font-medium shadow-inner">
                      {schema.ddl}
                    </pre>
                  </div>
                </div>
              ...
              {filteredSchemas.length === 0 && (
                <div className="text-center py-16 bg-[#FCFCFD] rounded-2xl border border-slate-200 text-slate-500 text-sm italic">
            ...
            <div className="max-h-[500px] overflow-y-auto space-y-8 pr-2">
              {apis.map((group, gIdx) => (
                <div key={gIdx} className="space-y-4">
                  <h4 className="text-slate-900 font-bold text-sm tracking-widest uppercase border-l-4 border-orange-500 pl-3">
                    {group.section}
                  </h4>
                  
                  {group.endpoints.map((api, kIdx) => (
                    <div key={kIdx} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:border-slate-300 transition-colors">
                      {/* EndPoint Head */}
                      <div className="bg-[#FCFCFD] px-5 py-4 flex flex-wrap gap-3 items-center justify-between border-b border-slate-200">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-mono text-sm font-bold text-slate-900">{api.url}</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="text-[11px] text-slate-500 font-mono tracking-wider bg-white px-2 py-1 border border-slate-200 rounded-md">Auth: <span className="font-bold text-slate-700">{api.auth}</span></span>
                        </div>
                      </div>
                      ...
                      <div className="p-5 grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                          <p className="text-slate-600 text-sm mb-4 leading-relaxed">{api.desc}</p>
                          <div className="space-y-2">
                            <span className="text-[10px] font-mono uppercase font-bold tracking-widest text-slate-500 block">Payload Schema:</span>
                            <pre className="font-mono text-xs text-slate-800 leading-relaxed bg-[#FCFCFD] p-4 rounded-lg border border-slate-100 overflow-x-auto whitespace-pre shadow-inner">
                              {api.payload}
                            </pre>
                          </div>
                        </div>
                        ...
                        <div>
                          <div className="space-y-2">
                            <span className="text-[10px] font-mono uppercase font-bold tracking-widest text-slate-500 block">Success Mock Response (200 OK):</span>
        ...
        {activeTab === 'security' && (
          <div className="space-y-6">
            <div className="bg-[#FCFCFD] p-6 rounded-2xl border border-slate-200 shadow-inner">
              <h4 className="text-slate-900 font-bold text-lg flex items-center gap-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-6">
                <div className="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm hover:border-slate-300 transition-colors">
            ...
            <div className="space-y-3 pt-2">
              <span className="text-xs font-mono uppercase font-bold tracking-widest text-slate-500 block">Typical MiddleWare Token Evaluation Code (Node/Express):</span>
              <pre className="font-mono text-xs text-slate-800 leading-relaxed bg-[#FCFCFD] p-5 rounded-2xl border border-slate-200 overflow-x-auto whitespace-pre shadow-inner">
